...
It handles startup, crash recovery, scheduled backups, and Discord event reporting — all driven by a central `config.json` file and a cross-linked controller architecture.
...
- [config_reference.md](config_reference.md) — Complete `config.json` key reference
