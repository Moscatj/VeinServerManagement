...
def build_dashboard(owner: "Main", dot_style) -> QtWidgets.QWidget:
    ...
    owner.btnBkView = QtWidgets.QPushButton("View Backups")
    set_button_role(owner.btnBkNow, BUTTON_SECONDARY)
    set_button_role(owner.btnBkView, BUTTON_SECONDARY)
    row.addWidget(owner.btnBkNow)
    row.addWidget(owner.btnBkView)
    row.addStretch(1)
    bkLay.addLayout(row, 2, 0, 1, 2)
    owner.btnBkView.clicked.connect(lambda: navigate("monitor.backups"))
