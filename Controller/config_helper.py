...
_raw_features = config.get("features")
if isinstance(_raw_features, dict):
...
def logs_dir() -> str:
    paths = paths_cfg()
    candidate = paths.get("logs_dir") or paths.get("logs") or config.get("logs_dir")
    return _norm_path(candidate)


def saves_dir() -> str:
    paths = paths_cfg()
    candidate = paths.get("saves_dir") or paths.get("saves") or config.get("save_dir")
    return _norm_path(candidate)


# Backups (save files)
def backups_cfg_v2() -> dict:
    b = (config.get("backups") or config.get("backup") or {}).copy()
    # normalize root
...
def log_snap_cfg() -> dict:
    ...
    logs.setdefault(
        "root", _norm_path((b.get("root") or "") + "\\Logs") if b.get("root") else ""
    )
...
def _migrate_backups_view() -> None:
    src = config.get("backups")
    if not isinstance(src, dict):
        src = config.get("backup")
    if not isinstance(src, dict):
        src = {}
    b = dict(src)
    config["backups"] = b
    ...
    # 1) enable: prefer backups.enabled/enable; otherwise adopt legacy features.enable_backups
    if "enable" not in b and "enabled" in b:
        b["enable"] = b["enabled"]
    if "enabled" not in b and "enable" in b:
        b["enabled"] = b["enable"]
    if "enable" not in b:
        legacy = bool(features.get("enable_backups", True))
        b["enable"] = legacy
    if "enabled" not in b:
        b["enabled"] = bool(b["enable"])
    ...
    if nightly:
        ret.setdefault(
            "Nightly",
            {
                ...
                "max_age_days": int(
                    nightly.get("max_backup_age_days", max(default_age, 30))
                ),
            },
        )
