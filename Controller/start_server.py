...
def _start_monitors() -> list[str]:
    ...
    if not wanted:
        send_discord_message(
            "ℹ️ Monitors disabled by config; none requested.", channel="startup"
        )
    elif not started:
        send_discord_message(
            "⚠️ Monitor spawn failed (see mgmt logs).", channel="startup"
        )
    else:
        send_discord_message(
...
def _steam_update_if_enabled() -> None:
    ...
    if ok is True:
        ...
        send_discord_message(
            "⚠️ Steam update failed; continuing with current build.", channel="startup"
        )
...
def main() -> int:
    ...
    pid_server_path = (
        PID_SERVER if "PID_SERVER" in globals() else (RUNTIME_DIR_PATH / "server.pid")
    )
    ...
    try:
        ...
        startup_quiet = int(
            (vcfg.raw.get("monitor", {}) or {}).get(
                "startup_quiet_seconds", vcfg.raw.get("startup_quiet_seconds", 120)
            )
        )
        if startup_quiet > 0:
        ...
        try:
            ...
            if not SELECTED_EXE.exists():
                ...
                if found:
                    ...
                    send_discord_message(
                        f"🔄 Falling back to {SELECTED_EXE}", channel="startup"
                    )
        ...
        _clear_restart_lock()
        send_discord_message(
            f"✅ Server process started (PID {proc.pid}). Waiting for joinable…",
            channel="startup",
        )
